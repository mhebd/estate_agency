...
function CreateTestimonial() {
  ...
  useEffect(() => {
    if (id) {
      setIsLoading(true);
      (async () => {
        const fetchTM = await fetchData(`/api/v1/testimonial/${id}`);
        setData(fetchTM.data);
        setIsLoading(false);
      })();
    }
  }, [id]);
  ...
  const submitHandler = async (e) => {
    ...
    if (id) {
      if (await updateData(`/api/v1/testimonial/${id}`, formData));
