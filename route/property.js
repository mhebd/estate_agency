...
router.route('/')
	.get(findAll)
	.post(private, limited('admin'), upload.fields([{name: 'images', maxCount: 5}]), handlePropertyImages, createOne);

router.route('/:id')
	.get(findOne)
	.put(private, limited('admin'), upload.fields([{name: 'images', maxCount: 5}]), handlePropertyImages, updateOne)
